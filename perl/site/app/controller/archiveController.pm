...
app::controller::archiveController - archive controller 
...
package app::controller::archiveController;
...
sub index {
    ...
    if(defined $self->{register}->get('mode')) {
        $self->{mode} = $self->{register}->get('mode');
    }
    
    my $test = $self->loadClass("test", 1, { "db" => $self->{db}, "id" => $self->{mode} } );
    
    $csvars->{text} = $test->getText();
